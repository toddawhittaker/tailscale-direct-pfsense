...
* Capture service command output through a temp file, never a command substitution. A command substitution puts the child's stdout on a pipe the daemon owns; a `SIGKILL` mid-start closes it and the orphaned `service` dies of `SIGPIPE` before finishing, leaving the service stopped. A file keeps the orphan alive to completion.
* `RESTART_CRITICAL`, `SHUTDOWN_PENDING`, and `RESTART_SETTLE_SECONDS_MAX` are internal state, not settings, and share a namespace with the sourced live config. `main` must re-assert them after `load_config_file`; `tests/test_config_validation.sh` asserts that statically.

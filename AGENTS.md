...
* The stop-to-start span is a critical section. `handle_shutdown` must record the request and return rather than exit while `RESTART_CRITICAL` is set, so the `start` always runs. Exiting between a stop and a start leaves Tailscale down on the router permanently: the watchdog then classifies every peer as `unknown`, which breaks the relayed sequence, so it never restarts the service. `RESTART_SETTLE_SECONDS` is capped below the rc wrapper's 5-second `SIGKILL` escalation for the same reason; raise one only with the other.

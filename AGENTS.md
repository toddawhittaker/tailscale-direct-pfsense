...
* `RESTART_CRITICAL`, `SHUTDOWN_PENDING`, `RESTART_SETTLE_SECONDS_MAX`, and `SERVICE_OUTPUT_TEMPLATE` are internal state, not settings, and share a namespace with the sourced live config. `main` must re-assert them after `load_config_file`; `tests/test_config_validation.sh` asserts that with an anchored whole-line match. Adding another such internal must extend both.
* Test fakes must fail loudly. A fake that swallows an error silently stops exercising its property, and assertions must be falsifiable by *replacement*, not only deletion — an unanchored `assert_contains` on a name is satisfied by prose that mentions it. Four assertions in this project have shipped unable to fail.

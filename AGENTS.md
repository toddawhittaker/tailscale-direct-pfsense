...
* The `SIGKILL` deadline is a single budget for the whole restart, not per service. `tailscale_watchdog_stop` in the rc wrapper escalates 10 seconds after `TERM` (the 5-second escalation in `kill_and_wait` is a different path, reachable only from a failed pidfile write). Settles not yet started are therefore skipped once a shutdown is pending, so at most one full settle sits on that path; `RESTART_SETTLE_SECONDS` is capped against that single pause. Raise the cap only alongside the wrapper's 10-second window, and keep the cap itself validated — it lives in the namespace the live config is sourced into.

...
Raising the cap requires raising the wrapper's 10-second window in step. `RESTART_SETTLE_SECONDS_MAX` is not an operator setting: it is derived from that window, and `main` re-asserts it immediately after the live config is sourced, along with `RESTART_CRITICAL` and `SHUTDOWN_PENDING`. Those three share a namespace with the config, so without the re-assert a config could raise the cap, or disable the check entirely by giving the bound a non-numeric value that makes the comparison error out instead of compare — and a config setting `RESTART_CRITICAL=1` would make `handle_shutdown` defer every signal for the life of the daemon, leaving `SIGKILL` as the only way to stop it.
